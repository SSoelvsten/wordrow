...
const Game = ({
  ...
  const timerSetting: DifficultyLogic = GetDifficultyLogic(
    difficulty,
    numberOfChars
  );
  ...
  const [latestGuessed, setLatestGuessed] = useState<string | null>(() => null);
  ...
  const [activatePressToContinue, setActivatePressToContinue] =
    useState<boolean>(() => false);
  const [isDrawn, setIsDrawn] = useState<boolean>(() => false);

  const currScore: number =
    (!guessed.includes(false) ? 2 : 1) *
    anagrams
      .filter((w, i) => guessed[i])
      .reduce((acc, w) => acc + scoreWord(w), 0);
  const qualified: boolean = !!guessed.find(
    (v, idx) => v && anagrams[idx].length === maxWordLength
  );
  ...
  const actionDelete = (idx: number = selectedLength - 1) => {
    ...
    setChars(
      chars.map(([c, i]) =>
        i === null || i === idx ? [c, null] : i < idx ? [c, i] : [c, i - 1]
      )
    );
  };
    ...
    if (emptySelection) {
      ...
      if (anagrams.includes(guess)) {
        ...
        if (guessedANewWord) {
          ...
          const remainingWords = guessed.reduce(
            (acc, v) => acc + (!v ? 1 : 0),
            0
          );
    ...
    if (gameEnd) {
      ...
      switch (e.key) {
        case ' ':
          ...
          break;
        case 'Backspace':
          e.ctrlKey || e.altKey ? actionClear() : actionDelete();
          break;
        case 'Escape':
          actionClear();
          break;
  ...
  const wordLengths: number[] = Array(maxWordLength - minWordLength + 1)
    .fill(0)
    .map((_, i) => i + minWordLength);
  ...
  const letterHeight =
    (LetterElement ? LetterElement.clientHeight : 2 * 5 + 16) + 1;
  ...
  const scoreboardElement = document
    .getElementsByClassName('ScoreBoard')
    .item(0);
  const scoreboardHeight = scoreboardElement
    ? scoreboardElement.clientHeight
    : 37;
  ...
  if (
    maxColumns <= wordColumns.length ||
    wordColumns.some((c) => maxInColumn <= c.length)
  ) {
      ...
      <div className={`Game`} tabIndex={0} onKeyDown={onKey} ref={divRef}>
...

        {
          <div className={`Anagrams`}>
            {wordColumns.map((c, i) =>
              c.map(([w, j], ci) => {
                const row = singleColumn
                  ? Math.floor(j % actualColumnSize) + 1
                  : ci + 1;
        ...
        <div className={`Bottom`}>
          {!gameEnd && (
            <>
              <div
                className={`Row ${guessed.includes(true) ? 'HasGood' : ''}`}
                key={latestGuessed}
              >
        ...
        <div className={`TopButtons Right`}>
          ...
          >
            <FontAwesomeIcon
              icon={gameEnd ? faSolid.faForwardStep : faSolid.faForward}
            />
          </button>
        </div>
      </div>
