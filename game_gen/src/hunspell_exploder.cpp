...
int main(int argc, char* argv[])
{
  ...
  const std::string dic_file_path = argv[1];
  const std::string aff_file_path = argv[2];
  ...
  // -----------------------------------------------------------------
  // Create hash set with results
  std::set<std::string> res;

  size_t generated_words = 0;

  const time_point dict__start_time = get_timestamp();
  while (d.can_pull()) {
    generated_words++;
    res.insert(d.pull());
  }
  const time_point dict__end_time = get_timestamp();

  std::cout << "Hunspell Depth-first:" << std::endl
            << "| #words      " << res.size() << std::endl
            << "| #duplicates " << (generated_words - res.size()) << std::endl
            << "| Time:       " << duration_of(dict__start_time, dict__end_time) << " ns" << std::endl;

  // -----------------------------------------------------------------
  // Output all words back to disk
  std::ofstream out_file(dic_file_path + ".exploded");

  const time_point output__start_time = get_timestamp();
  for (const std::string &w : res) {
    if (!regex_match(w, std::regex("[a-zæøå]*"))) continue;

    out_file << w << std::endl;
  }
  const time_point output__end_time = get_timestamp();

  std::cout << "Output to Disk:" << std::endl
            << "| Time:    " << duration_of(output__start_time, output__end_time) << " ns" << std::endl;
}
